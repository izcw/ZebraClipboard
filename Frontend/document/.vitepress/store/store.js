...
const persistConfig = {
  ...
  strategies: [
    {
      key: 'store', // 存储名称
      storage: 'sessionStorage', // 存储方式，可以是 'localStorage' 或 'sessionStorage'
      paths: ['counter'] // 指定 state 中哪些数据需要被持久化，如果为空或者不存在等就全部存储
    }
  ]
};

const piniaName = 'store';

export const piniaStore = defineStore(piniaName, {
  state: () => ({
    data: {
      userName: "张三",
      id: "467589ghdsgf893486"
    },
    counter: 0,
  }),
  actions: {
